...
function getCode(){
        ...
        btnHint.addEventListener("click", ()=>{
          ...
          try{
            ...
            if(!open && ed && window.CodeMirror){
              window.CodeMirror.showHint(ed, window.__customPythonHint || undefined, {completeSingle:false});
            }
          }catch(e){}
        });
      }
  ...
  function patchRunTestsRubric(){
    ...
    window.runTests = async function(){
      ...
      // ---- PASS theo cấu trúc (không bắt buộc khớp output từng chữ) ----
      const needInput = !/không\s*có|khong\s*co|không|khong/i.test(String(lesson.input||""));
      const hasPrint = /\bprint\s*\(/.test(code);
      const hasInput = /\binput\s*\(/.test(code) || /sys\.stdin|stdin\.read|readline\s*\(/i.test(code);

      let missing = [];
      if(!hasPrint) missing.push("print(...)");
      if(needInput && !hasInput) missing.push("input()");
      ...
      if(okRun && okStruct){
        ...
        $("console").textContent =
          `✅ PASS (Theo cấu trúc)\n` +
          `- Điều kiện: ${needInput ? "có input() + print(...)" : "có print(...)"}\n` +
